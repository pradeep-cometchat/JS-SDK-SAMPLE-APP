...
import {
  CURRENT_USER, THREAD_MESSAGES, INITIAL_MESSAGES, Centered, noop,
} from '../_helpers';

export default {
  title: 'Web/Messages/Thread View',
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'Side panel for threaded replies — parent message, scrollable reply list, and a composer. Each reply supports reactions via hover (desktop) or long-press (mobile).',
      },
    },
  },
};
...
export const WithReplies = Default;
        ...
        <ThreadPanel
          ...
          onReactThread={(replyId, emoji) => setReplies(prev => prev.map(r => {
            ...
            if (idx === -1) reactions.push({ emoji, userIds: [CURRENT_USER.id] });
              ...
              rx.userIds = rx.userIds.includes(CURRENT_USER.id) ? rx.userIds.filter(u => u !== CURRENT_USER.id) : [...rx.userIds, CURRENT_USER.id];
