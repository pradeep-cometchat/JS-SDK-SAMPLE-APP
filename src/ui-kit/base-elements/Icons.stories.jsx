...
export default {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'Every icon is a Material Symbols Rounded glyph rendered via the internal `MIcon` helper. Each component accepts a `size` prop (pixels).',
      },
    },
  },
};

const ICON_COMPONENTS = Object.entries(Icons).filter(
  ([name, value]) => typeof value === 'function' && /Icon$/.test(name),
);
      ...
      <div style={{ background: 'var(--surface)', padding: 24, borderRadius: 12, border: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
          {ICON_COMPONENTS.map(([name, C]) => (
            <div key={name} style={{ padding: 14, background: 'var(--bg)', borderRadius: 8, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, color: 'var(--text)' }}>
...
export const Sizes = {
  render: () => (
    <Centered maxWidth={640}>
      <div style={{ background: 'var(--surface)', padding: 24, borderRadius: 12, border: '1px solid var(--border)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 32 }}>
